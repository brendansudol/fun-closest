...
import { formatCountdownLabel, formatPackLabel } from "@/lib/game/format";
import {
...
import type { HostRoomState, Pack } from "@/types/game";
...
export function HostRoomScreen({ slug }: { slug: string }) {
  ...
  const startMutation = useMutation({
    mutationFn: () =>
      requestJson(`/api/inkling/rooms/${slug}/host/start`, {
        method: "POST",
        body: JSON.stringify({
          pack: activePack,
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["game", "host-room", slug] });
    },
  });

  const lockMutation = useMutation({
    mutationFn: () =>
      requestJson(`/api/inkling/rooms/${slug}/host/lock`, {
        method: "POST",
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["game", "host-room", slug] });
  ...
  const swapMutation = useMutation({
    mutationFn: () =>
      requestJson(`/api/inkling/rooms/${slug}/host/swap`, {
        method: "POST",
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["game", "host-room", slug] });
  ...
  const revealMutation = useMutation({
    mutationFn: () =>
      requestJson(`/api/inkling/rooms/${slug}/host/reveal`, {
        method: "POST",
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["game", "host-room", slug] });
  ...
  const resetMutation = useMutation({
    mutationFn: () =>
      requestJson(`/api/inkling/rooms/${slug}/host/reset`, {
        method: "POST",
      }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["game", "host-room", slug] });
    ...
    <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
      ...
      <aside className="grid gap-6 self-start">
        ...
        <section className="glass-card rounded-[2rem] p-5">
          ...
          <Link
            href={`/inkling/join/${data.room.joinCode}`}
