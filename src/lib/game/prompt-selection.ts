...
import type { RoundStore, Pack, Prompt } from "@/types/game";
...
export function getRoundPromptHistoryIds(round: Pick<RoundStore, "promptId" | "promptHistoryIds">) {
...
export function getPromptRevision(round: Pick<RoundStore, "promptId" | "promptHistoryIds">) {
  return getRoundPromptHistoryIds(round).length - 1;
}

export function listUsedPromptIdsForRounds(rounds: Array<Pick<RoundStore, "promptId" | "promptHistoryIds">>) {
