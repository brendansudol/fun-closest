...
export function parseGuessInput(rawInput: string) {
  ...
  if (!normalizedInput) {
    throw new GameError(400, "Enter a numeric guess.");
  }
  ...
  if (!Number.isFinite(numericValue)) {
    throw new GameError(400, "That guess could not be parsed.");
  }

  const normalizedValue = Math.round(numericValue * multiplier * 10_000) / 10_000;

  if (normalizedValue <= 0) {
    throw new GameError(400, "Guesses must be greater than zero.");
  }
