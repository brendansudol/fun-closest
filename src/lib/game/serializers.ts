...
function buildRoomMeta(room: RoomStore, roomVersion: number) {
  return {
    slug: room.slug,
    title: room.title || DEFAULT_ROOM_TITLE,
    joinCode: room.joinCode,
    joinPath: `/inkling/join/${room.joinCode}`,
...
function buildScoreboard(players: PlayerStore[], meId?: string) {
  return [...players]
    .sort((left, right) => {
...
function formatRoundNumericValue(
  value: number,
  round: Pick<RoundStore, "category" | "promptUnitLabel" | "promptUnitShort">,
) {
...
function getRoundAnswerDisplay(round: RoundStore) {
...
function buildResults(round: RoundStore, players: PlayerStore[], guesses: GuessStore[], meId?: string) {
